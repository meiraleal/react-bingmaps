    ...
    class ReactBingmaps extends Component {
      ...
      componentWillReceiveProps(nextProps){
        ...
        if(this.props.Polygons !== nextProps.Polygons){
          this.createPolygons(nextProps.Polygons, mapReference);
        }
      ...
      reactBingmaps(props,  Microsoft){
        ...
        if(bingmapKey && Microsoft){
          ...
          this.createPolygons(Polygons, mapReference);
      ...
      createPolygons(polygons, mapReference){
        if(map[mapReference] && polygons){
          for (var i = map[mapReference].entities.getLength() - 1; i >= 0; i--) {
                var polygon = map[mapReference].entities.get(i);
            ...
            for(let polygonIndex = 0; polygonIndex < polygons.length; polygonIndex++){
              if(polygons[polygonIndex][0] &&
                polygons[polygonIndex][1])
              {
                let location =  new Microsoft.Maps.Location(regularPolygons[regularPolygonIndex][0], regularPolygons[regularPolygonIndex][1]);
                Microsoft.Maps.loadModule('Microsoft.Maps.SpatialMath', function () {
                  var locations = Microsoft.Maps.SpatialMath.getRegularPolygon(location, radius, points, Microsoft.Maps.SpatialMath.DistanceUnits.Miles);
                  var polygon = new Microsoft.Maps.Polygon(locations, option);
                  map[mapReference].entities.push(polygon);
